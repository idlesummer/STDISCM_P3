...
public:
    ...
    void onMount() override {
        ...
        useEffect([this]() {
            std::cout << "Player health: " << health.get().x << std::endl;
            return []() {};  // No cleanup
        }, {});
    }
    ...
    std::shared_ptr<RenderNode> render() override {
        ...
        healthTextProps["content"] = "HP: " + std::to_string((int)health.get().x);
    ...
    void updateHealth(int newHealth) {
        health.set(sf::Vector2f(newHealth, 0));  // Using Vector2f to store int (hack for demo)
    }
...
private:
    ...
    StateRef<sf::Vector2f> health;  // Using Vector2f to store health (demo simplification)
};
